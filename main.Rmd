...
if(!require(car)){install.packages('car')} # for qqPlot
library(car)
...
grouped_data <-  measured_data %>%
  ...
  summarise_at(
    vars(firstContactEndTrialElapsed, logFirstContactEndTrialElapsed),
    mean)
...
```{r}
pairwise.t.test(
  grouped_data$logFirstContactEndTrialElapsed,
...
```{r}
#wilcox.test(grouped_data_keyboard$firstContactEndTrialElapsed, grouped_data_keyboard$firstContactEndTrialElapsed, paired = TRUE, alternative = "two.sided")
```
